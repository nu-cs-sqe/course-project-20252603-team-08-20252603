...
|                    | State of the System                                      | Expected output                                                                                                                                      | Implemented? |
|--------------------|----------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| Integration TC 4   | Three separate new `Game` instances; `startGame(2, US)`, `startGame(3, US)`, `startGame(4, US)` each succeeds | 2-player game: gem token count per color == 4, `getRevealedNobles().size() == 3`; 3-player game: gem token count per color == 5, nobles == 4; 4-player game: gem token count per color == 7, nobles == 5 | :white_check_mark: |
